...
class MemeEditing extends Component {
  ...
  postMeme() {
    ...
    if (this.props.user && this.props.user.username) {
      ...
      this.setState({
        ...
        axios.post(ENV.REACT_APP_BACKEND+'/api/postMeme', {meme, user:this.props.user}).then((response)=>{
  ...
  render() {
    ...
    if (state.preview && state.image) {
      image = (
        <img src={state.image.Location} style={{width:500, height: 'auto' }}/>
      )
    } else if (state.image) {
      image = (
        <Cropper
      ...
      image = (
        <Dropzone title='Add Image' style={{width:'440px', height: '420px', margin:'0 auto', background:`url(${UploadIcon})`, backgroundSize:'cover', backgroundRepeat: 'no-repeat', backgroundPosition:'center center'}} multiple={false} accept="image/*" onDrop={(e)=>this.handleFile(e)}/>
      );
    }
    if (this.state) {
      caption = (
            ...
            <div className='meme-editing-select-container'>
              <MultiSelect className='multiselect' maxLength={28} style={{minWidth: '300px', maxWidth:'300px', color:'white'}} theme='material' maxValues = {5} 
                createFromSearch = {(options, values, search)=>{
                  ...
                  if (search.length > 1 && search.length < 29) {
                    return {
...
export default connect(mapStateToProps)(MemeEditing);